...
import Parking.Tickets.TicketHandler;
...
class Floor
{
		...
		for(int i = 0 ; i < slots.length ; i++)
		{
			if(slots[i].getType() == type && slots[i].isFree())
			{
				if(slots[i].parkVehicle(v, ticket +"_"+(i+1))) 
				{
					...
					ticketHandler.parkVehicle(v.getNumber(), intTypeToString(v.getType()), ticket +"_"+(i+1));
				}
	...
	{
		Vehicle v = slots[slot-1].getVehicle();
		if(slots[slot-1].unparkVehicle())
		{
			System.out.println("Unparked Vehicle");
			ticketHandler.unparkVehicle(v.getNumber(), intTypeToString(v.getType()), v.getTicket());
			return true;
		}
